...
class Solver(lib.Solver):
    ...
    def verify_update(
        self, update: ManualUpdate, rules: dict[int, list[PageOrderRule]]
    ) -> bool:
    ...
    def reshuffle_update(
        ...
        while len(corrected_pages) < len(update.pages) and changed:
            ...
            for page in update.pages:
                ...
                for rule in rules[page]:
                    if (
                        rule.predecesor in update.pages
                        and rule.predecesor not in corrected_pages
                    ):
                        self.printd(
                            f"Page {page:2d}. Awaiting: {rule.predecesor=} not added yet"
                        )
